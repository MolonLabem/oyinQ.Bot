# Очистка и проверка использования кода

## Текущий проход — 2026-09-24

Исходная точка: `master` / `origin/master` на `c8cfabe` после fetch, расхождение
0/0. Незакоммиченный UX-проход сохранён; его фильтр мест, изменения хотелок,
профиля и обновления списка не относятся к удалённому коду этого прохода.

Проверены `Program.cs` (DI, три CLI-команды, bootstrap, webhook/polling), группы
Mini App endpoints, Telegram routing/callbacks, граф импортов от `main.tsx`,
фоновые службы, EF/design-time вход, пакеты, конфигурация, Docker/CI и скрипты.
Все рабочие TS/TSX-модули достижимы по статическим импортам; декларации Vite и
Telegram подключает TypeScript. Новых неиспользуемых компонентов не обнаружено.

Удаления подтверждены проверкой вызовов, импортов и динамических механизмов:

- Приватный `BoardGameGeekClient.ReadTags` не вызывается. `ParseThing` использует
  `ReadTaxonomy`, сохраняющий BGG ID; парсинг метаданных остаётся прежним.
- `uniqueByBggId` и `campCommitmentLabels` не имеют потребителей. Реальные
  `mergeGameSearchCandidates` / `mergeExpansions` и тексты элементов UI сохранены.
- `CampParticipantAdminService.BuildMessages` вызывался только двумя тестами.
  Они теперь проверяют `CampParticipantMessages.Build`, используемый отправкой;
  проверки полноты, разбиения и лимита сообщений сохранены.
- CSS для `section-divider`, `contribution-row`, `wish-actions` не используется
  ни в JSX, ни при динамическом назначении классов. Из общих селекторов удалены
  только эти ветви, действующие `collection-row` / `wish-filters` сохранены.

Общий хук мутаций вынесен из страницы в `hooks/useCampWishlistAction.ts` без
изменения POST, состояний ошибки/занятости и события успешного сохранения.
Профиль импортирует его и `useRefreshOnActivity` напрямую; реэкспорт через
`CampWishlist` удалён. Доменные правила и владельцы транзакций не менялись.
В `AGENTS.md` исправлен устаревший путь к собственным хотелкам.

Зависимости, lock-файл, конфигурация, переменные окружения, маршруты, DTO и
миграции не менялись; действий при обновлении конфигурации нет. EF Design,
VS Container Tools, прямые EF-ссылки и все npm-пакеты имеют потребителей.
Отдельные workers, legacy owner bootstrap, старые callbacks/deep links,
`tab=mine`, import storage keys, исторические snapshots и `docs/releases/**`
сохранены: отсутствие вызова из текущего экрана не доказывает их ненужность.
Отслеживаемого сборочного мусора для безопасного удаления не обнаружено.

Проверки до/после: сервер — 732 passed, 35 skipped; Mini App — 233 passed
(50 файлов). Итоговая Release-сборка: 0 ошибок/предупреждений; strict TypeScript
и Vite build проходят. Полный набор включает соответствие EF-модели последнему
snapshot. `verify-release-input.ps1` проходит. Временная изолированная проверка
реальных регистраций `Program.cs` с ValidateOnBuild/ValidateScopes собрала
220 регистраций и 107 маршрутов Mini App в обоих режимах Telegram. Это проверка
DI и построения маршрутов, не полный запуск приложения с БД: host, workers и
миграции не запускались, использовались только фиктивные настройки.

Мобильные проверки Chromium с подставным API прошли на ширинах 360/390/430
в светлой и тёмной темах и при высоте 460: профиль/коллекция, длинные названия,
фильтры, порционная выдача, переходы игра → участник → игра → назад, сохранение
прокрутки, ошибка обновления с сохранённым списком, пустой поиск. Ошибок JS и
мутаций при просмотре нет; снимки профиля/коллекции на 360 визуально проверены.
Это браузерная эмуляция, не проверка Telegram на устройстве.

35 PostgreSQL-тестов пропущены из-за отсутствия `OYINQ_TEST_POSTGRES` как до,
так и после изменений. Docker не запускался из-за известной проблемы Hypervisor
на этой машине; Dockerfile и CI проверены статически. Lint-скрипта в проекте нет.
Live Telegram/BGG, production и применение миграций не проверялись. Ниже —
исторический отчёт: его результаты PostgreSQL/Docker не являются проверками
этого прохода.

# Исторический проход — 2026-09-16
