...
Проверить пункты 9–14 в [планировании и партиях](planning-notifications-plays.md): отрицательный исход без партии, фактический состав и несколько ссылок BG Stats, внешнюю игру с дополнениями и явным владением, атомарность локальных изменений, предпросмотр рассылки и повтор только ошибочных получателей. Текущий текст для Telegram автоматически берётся из последней датированной записи `CHANGELOG.md`. Сверить предпросмотр с журналом; после правки в тот же день старый предпросмотр должен требовать обновления, а история ранее отправленных сообщений — сохраняться.

Проверить административный запрос набора по [спецификации](admin-recruitment.md): активный Camp, общий интервал с организатором, отказ без прав и при отсутствии подходящих сборов. В деталях Camp проверить один выделенный блок коробки с несколькими владельцами и одну кнопку возврата в Telegram; в обычном браузере возврат остаётся внутри страницы. Проверить открытие старых ссылок после обновления по [политике кеширования](miniapp-caching.md).
