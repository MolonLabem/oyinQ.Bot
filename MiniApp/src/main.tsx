...
function AdminCommunitiesView({ values, onChange, close }: { values: AdminCommunity[]; onChange: (values: AdminCommunity[]) => void; close: () => void }) {
  ...
  useEffect(() => { void api<AdminClub[]>("/api/miniapp/admin/clubs").then(result => { setClubs(result); if (result[0]) { setSelectedClub(result[0].id); setBggUsername(result[0].bggUsername ?? ""); } }); }, []);
