...
type AdminClub = { id: number; botChatKey: string; name: string; gameCount: number };
...
function AdminCommunitiesView({ values, onChange, close }: { values: AdminCommunity[]; onChange: (values: AdminCommunity[]) => void; close: () => void }) {
  ...
  useEffect(() => { void api<AdminClub[]>("/api/miniapp/admin/clubs").then(result => { setClubs(result); if (result[0]) setSelectedClub(result[0].id); }); }, []);
  const loadCollection = async (clubId = selectedClub) => { if (!clubId) return; const document = await api<object>(`/api/miniapp/admin/clubs/${clubId}/collection`); setCollectionJson(JSON.stringify(document, null, 2)); };
  const replaceCollection = async () => { try { await api(`/api/miniapp/admin/clubs/${selectedClub}/collection`, { method: "PUT", body: collectionJson }); setError(""); } catch (e) { setError((e as Error).message); } };
  ...
  return <main><button className="back" onClick={close}>← Назад</button><header><div><span className="eyebrow">АДМИНИСТРИРОВАНИЕ</span><h1>Сообщества</h1></div></header>
    ...
    <section className="form"><h2>Коллекция клуба</h2><label>Клуб<select value={selectedClub} onChange={event => { const id = Number(event.target.value); setSelectedClub(id); void loadCollection(id); }}>{clubs.map(club => <option value={club.id} key={club.id}>{club.name} ({club.gameCount})</option>)}</select></label><div className="actions"><button onClick={() => loadCollection()}>Показать / экспортировать JSON</button><input inputMode="numeric" value={addBggId} placeholder="BGG ID" onChange={e => setAddBggId(e.target.value)} /><button disabled={!selectedClub || !addBggId} onClick={addGame}>Добавить игру BGG</button></div>{collectionJson && <><textarea rows={16} value={collectionJson} onChange={e => setCollectionJson(e.target.value)} /><button className="primary" onClick={replaceCollection}>Проверить и заменить JSON</button></>}</section>
