...
export class ApiError extends Error {
  constructor(message: string, public status: number, public code?: string, public currentRevision?: number) { super(message); }
}
...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(body.message ?? "Не удалось выполнить запрос.", response.status, body.code, body.currentRevision);
  }
