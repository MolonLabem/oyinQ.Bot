...
function GatheringList({ community, open, create }: { community: Community; open: (id: string) => void; create: () => void }) {
  ...
  return <Page title="Сборы" subtitle={community.name} actions={<button className="primary" onClick={create}>Создать сбор</button>}>
    ...
    {state.loading ? <Loading /> : state.error ? <ErrorState message={state.error} retry={state.reload} /> : !state.data?.length ? view === "upcoming" ? <><Empty>Пока нет предстоящих сборов.</Empty><button className="primary" onClick={create}>Создать сбор</button></> : <Empty>Здесь появятся сыгранные и отменённые сборы.</Empty> :
...
function GatheringDetails({ community, id, onBack }: { community: Community; id: string; onBack: () => void }) {
  ...
  return <Page title={value.gathering.gameName} subtitle={value.gathering.localDateTime} actions={<button onClick={onBack}>Назад</button>}>
    ...
    <Card><h2>Участники</h2><ul>{value.confirmedParticipants.map((p, i) => <li key={`${p.name}-${i}`}>{p.contactUrl ? <a href={p.contactUrl}>{p.name}</a> : <span title="У пользователя нет публичного username Telegram">{p.name}</span>}{p.isOrganizer ? " — организатор" : ""}</li>)}</ul>{value.waitlistedParticipants.length > 0 && <><h3>Лист ожидания</h3><ol>{value.waitlistedParticipants.map(p => <li key={`${p.position}-${p.name}`}>{p.contactUrl ? <a href={p.contactUrl}>{p.name}</a> : <span title="У пользователя нет публичного username Telegram">{p.name}</span>}</li>)}</ol></>}</Card>
