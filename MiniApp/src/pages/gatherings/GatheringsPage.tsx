...
export function GatheringsPage({ community, initialGatheringId }: { community: Community; initialGatheringId?: string }) {
  ...
  if (screen === "create") return <CreateGathering community={community} onDone={() => setScreen("list")} />;
...
function CreateGathering({ community, onDone }: { community: Community; onDone: () => void }) {
  ...
  const [description, setDescription] = useState(""); const [teach, setTeach] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState<string>();
  ...
  async function submit() {
    ...
    try { await api("/gatherings", json("POST", { communityKey: community.key, gameSource: source, bggId: chosen.bggId, selectedExpansionIds: expansions, startsAtLocal: starts, minimumPlayers: minimum, desiredPlayers: desired, maximumPlayers: maximum, description, canTeachRules: teach })); telegram.success(); onDone(); }
    catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setBusy(false); }
  }
  return <Page title="Новый сбор" actions={<button onClick={onDone}>Назад</button>}>
    <div className="segmented"><button className={source === "catalog" ? "active" : ""} onClick={() => { setSource("catalog"); setChosen(undefined); }}>Из каталога</button><button className={source === "bgg" ? "active" : ""} onClick={() => { setSource("bgg"); setChosen(undefined); }}>По ссылке BGG</button></div>
    ...
    {error && <Notice kind="danger">{error}</Notice>}<button className="primary sticky-action" disabled={busy || !chosen} onClick={submit}>{busy ? "Создаём…" : "Создать сбор"}</button>
  </Page>;
}
...
function GatheringDetails({ community, id, onBack }: { community: Community; id: string; onBack: () => void }) {
  ...
  const [busy, setBusy] = useState(false); const [error, setError] = useState<string>(); const [editing, setEditing] = useState(false);
  async function action(path: string, reason?: string) { setBusy(true); setError(undefined); try { await api(`/gatherings/${id}/${path}`, json("POST", { communityKey: community.key, reason })); telegram.success(); state.reload(); } catch (e) { setError(e instanceof Error ? e.message : String(e)); } finally { setBusy(false); } }
  ...
  return <Page title={value.gathering.gameName} subtitle={value.gathering.localDateTime} actions={<button onClick={onBack}>Назад</button>}>
    ...
    {error && <Notice kind="danger">{error}</Notice>}<div className="action-bar">{value.canJoin && <button className="primary" disabled={busy} onClick={() => action("join")}>Присоединиться</button>}{value.canLeave && <button className="danger" disabled={busy} onClick={() => action("leave")}>Отказаться от места</button>}{value.canManage && <><button className="primary" onClick={() => setEditing(true)}>Изменить сбор</button>{value.status !== "Closed" && <button disabled={busy} onClick={() => action("close")}>Закрыть запись</button>}{value.status === "Closed" && <button onClick={() => action("reopen")}>Открыть запись</button>}<button className="danger" onClick={async () => { const reason = prompt("Причина отмены (необязательно)") ?? undefined; if (await telegram.confirm("Отменить сбор?")) action("cancel", reason); }}>Отменить сбор</button></>}</div>
  </Page>;
}
