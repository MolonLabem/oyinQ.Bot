...
import { Badge, ContactLink, Cover, Empty, ErrorState, Field, Loading, Notice, Page, Tabs } from "../../components/Ui";
...
export function GamesPage({ community, bggAvailable, initialGameId, onInitialConsumed, backToGathering }: { community: Community; bggAvailable: boolean; initialGameId?: number; onInitialConsumed?: () => void; backToGathering?: () => void }) {
  ...
  const clearFilters = () => { setOwnership(""); setProviderParticipantIds([]); setAvailability(""); setPlanning(""); setPlayers(undefined); setTypes([]); setCategories([]); setSort("name"); };
...
function GameCard({ game, open, club }: { game: GameListItem; club: boolean; open: () => void }) {
  return <button className="card catalog-card" onClick={open}><Cover src={game.thumbnailImageUrl} name={game.name} /><span className="catalog-card-body"><strong className="catalog-title">{game.name}</strong><span className="catalog-card-facts">{game.minPlayers && game.maxPlayers && <small><span aria-hidden>👥</span> {game.minPlayers}–{game.maxPlayers}{game.bestPlayers ? ` · лучше ${game.bestPlayers}` : ""}</small>}{Boolean(game.scheduledGatherings) && <small className="catalog-activity">Запланировано: {game.scheduledGatherings}</small>}</span>{game.availabilitySummary && <small className={game.needsProviderCoordination ? "availability warning" : game.isDefinitelyAvailable ? "availability success" : "availability"}>{game.availabilitySummary}</small>}</span></button>;
}
...
function GameDetail({ community, bggId, back }: { community: Community; bggId: number; back: () => void }) {
  ...
  if (state.failure instanceof ApiError && state.failure.code === "game_not_in_collection") return <Page title="Игра" subtitle={community.name} actions={<button className="ghost page-back" onClick={back}><span aria-hidden>← </span>Назад</button>}><Notice kind="warning">{collectionMissingMessage(community)}</Notice></Page>;
  if (state.error || !state.data) return <Page title="Игра" actions={<button className="ghost page-back" onClick={back}><span aria-hidden>← </span>Назад</button>}><ErrorState message={state.error ?? "Игра не найдена"} retry={state.reload} /></Page>;
  ...
  return <Page title={game.name} subtitle={game.yearPublished ? `${game.yearPublished} год` : undefined} actions={<button className="ghost page-back" onClick={back}><span aria-hidden>← </span>Назад</button>}>
