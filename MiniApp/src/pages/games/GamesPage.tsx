...
function GameCard({ game, open }: { game: GameListItem; open: () => void }) {
  return <button className="card catalog-card" onClick={open}><Cover src={game.thumbnailImageUrl} name={game.name} /><span className="catalog-card-body"><strong className="catalog-title">{game.name}</strong><span className="tag-list"><Badge tone="accent">{game.typeName}</Badge>{game.categoryNames.slice(0, 2).map(name => <span className="tag" key={name}>{name}</span>)}</span>{game.minPlayers && game.maxPlayers && <small>👥 {game.minPlayers}–{game.maxPlayers}{game.bestPlayers ? ` · лучше ${game.bestPlayers}` : ""}</small>}{game.availabilitySummary && <small className={game.needsProviderCoordination ? "availability warning" : game.isDefinitelyAvailable ? "availability success" : "availability"}>{game.availabilitySummary}</small>}</span></button>;
}
...
function GameDetail({ community, bggId, back }: { community: Community; bggId: number; back: () => void }) {
  ...
  return <Page title={game.name} subtitle={game.yearPublished ? `${game.typeName} · ${game.yearPublished}` : game.typeName} actions={<button onClick={back}>Назад</button>}>
    ...
    <Card className="detail-section"><h2>Доступность</h2>{game.availability.isInBaseCollection && <Notice kind="success">✓ {community.mode === "Camp" ? "Базовая коллекция кэмпа" : "Коллекция клуба"}</Notice>}{game.availability.providers.length > 0 && <><h3>Кто может привезти</h3><ul className="provider-list">{game.availability.providers.map(provider => <li key={provider.participantId}><span>{provider.commitment === "Bringing" ? "✅ " : ""}{provider.contactUrl ? <a href={provider.contactUrl}>{provider.displayName}</a> : <span title="У пользователя нет публичного username Telegram">{provider.displayName}</span>}{provider.city ? ` (${provider.city})` : ""}</span><span>{provider.commitment === "Bringing" ? "точно привезёт" : "может привезти"}</span></li>)}</ul></>}{!game.availability.isInBaseCollection && !game.availability.hasCommittedProvider && game.availability.providers.length > 1 && <Notice kind="warning">Нужно решить, кто привезёт игру.</Notice>}</Card>
