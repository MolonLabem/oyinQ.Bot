...
function GameCard({ game, open, club }: { game: GameListItem; club: boolean; open: () => void }) {
  return <button className="card catalog-card" onClick={open}><Cover src={game.thumbnailImageUrl} name={game.name} /><span className="catalog-card-body"><strong className="catalog-title">{game.name}</strong><small>Запланировано сборов: {game.scheduledGatherings ?? 0} · Подтверждено партий: {game.recordedPlays ?? 0}</small><span className="tag-list">{game.typeNames.map(name => <Badge tone="accent" key={name}>{name}</Badge>)}</span>{game.minPlayers && game.maxPlayers && <small>👥 {game.minPlayers}–{game.maxPlayers}{game.bestPlayers ? ` · лучше ${game.bestPlayers}` : ""}</small>}{game.availabilitySummary && <small className={club ? "muted" : game.needsProviderCoordination ? "availability warning" : game.isDefinitelyAvailable ? "availability success" : "availability"}>{game.availabilitySummary}</small>}</span></button>;
}
...
function GameDetail({ community, bggId, back }: { community: Community; bggId: number; back: () => void }) {
  ...
  return <Page title={game.name} subtitle={game.yearPublished ? `${game.yearPublished} год` : undefined} actions={<button onClick={back}>Назад</button>}>
    ...
    {game.expansions.length > 0 && <Card className="detail-section"><details><summary>Дополнения в коллекции ({game.expansions.length})</summary><ul>{game.expansions.map(item => <li key={item.bggId}>{item.name}</li>)}</ul></details></Card>}
  </Page>;
}
