...
export function GamesPage({ community }: { community: Community }) {
  ...
  return <Page title="Игры" subtitle={state.data ? `${community.name} · ${state.data.items.length}` : community.name}>
    ...
    {state.loading ? <Loading /> : state.error ? <ErrorState message={state.error} retry={state.reload} /> : !state.data?.items.length ? <Empty>{query.trim() || activeFilters ? "Игры не найдены. Измените поиск или фильтры." : community.mode === "Club" ? "В коллекции пока нет игр. Администратор может добавить их через управление коллекцией." : "На кэмпе пока нет доступных игр. Добавьте личную игру в разделе «Мои игры»."}</Empty> : <div className="catalog-grid">{state.data.items.map(game => <GameCard key={game.bggId} game={game} open={() => setSelected(game.bggId)} />)}</div>}
  </Page>;
}
