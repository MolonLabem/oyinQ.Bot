...
export function GamesPage({ community }: { community: Community }) {
  ...
  const games = useMemo(() => state.data?.filter(game => game.name.toLowerCase().includes(query.toLowerCase())) ?? [], [state.data, query]);
  return <Page title="Игры" subtitle={community.name}>
    ...
    {state.loading ? <Loading /> : state.error ? <ErrorState message={state.error} retry={state.reload} /> : !games.length ? <Empty>Игры не найдены.</Empty> : <div className="game-list">{games.map(game => <Card key={game.bggId}><div className="media"><Cover src={game.thumbnailImageUrl} name={game.name} /><div><h2>{game.name}</h2>{game.copyCount && <p>{game.copyCount} коп.</p>}{game.providers?.length ? <ul className="providers">{game.providers.map((p, i) => <li key={`${p.displayName}-${i}`}>{p.displayName}</li>)}</ul> : null}{game.expansions?.length ? <p className="muted">Дополнения: {game.expansions.map(e => e.name).join(", ")}</p> : null}</div></div></Card>)}</div>}
  </Page>;
}
