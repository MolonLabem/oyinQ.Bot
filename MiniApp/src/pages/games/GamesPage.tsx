...
export function GamesPage({ community, initialGameId, onInitialConsumed, backToGathering }: { community: Community; initialGameId?: number; onInitialConsumed?: () => void; backToGathering?: () => void }) {
  ...
  return <Page title="Игры" subtitle={state.data ? `${community.name} · ${state.data.items.length}` : community.name}>
    ...
    {state.loading ? <Loading /> : state.error ? <ErrorState message={state.error} retry={state.reload} /> : !state.data?.items.length ? <Empty>{query.trim() || activeFilters ? "Ничего не найдено. Попробуйте изменить запрос или фильтры." : community.mode === "Club" ? "В коллекции пока нет игр. Администратор может добавить их в разделе управления коллекцией." : "На кэмпе пока нет доступных игр. Добавьте свою игру в разделе «Профиль → Моя коллекция»."}</Empty> : <div className="catalog-grid">{state.data.items.map(game => <GameCard key={game.bggId} game={game} open={() => setSelected(game.bggId)} />)}</div>}
  </Page>;
}
...
function GameCard({ game, open }: { game: GameListItem; open: () => void }) {
  return <button className="card catalog-card" onClick={open}><Cover src={game.thumbnailImageUrl} name={game.name} /><span className="catalog-card-body"><small>Запланировано сборов: {game.scheduledGatherings ?? 0} · Подтверждено партий: {game.recordedPlays ?? 0}</small><strong className="catalog-title">{game.name}</strong><span className="tag-list">{game.typeNames.map(name => <Badge tone="accent" key={name}>{name}</Badge>)}</span>{game.minPlayers && game.maxPlayers && <small>👥 {game.minPlayers}–{game.maxPlayers}{game.bestPlayers ? ` · лучше ${game.bestPlayers}` : ""}</small>}{game.availabilitySummary && <small className={game.needsProviderCoordination ? "availability warning" : game.isDefinitelyAvailable ? "availability success" : "availability"}>{game.availabilitySummary}</small>}</span></button>;
}
