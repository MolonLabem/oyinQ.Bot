...
export function CommunityDemand({ communityKey, create }: { communityKey: string; create?: (id: number) => void }) {
  ...
  return <section className="content-section"><h2>Во что хотят сыграть в сообществе</h2>
    ...
    <button className="ghost" onClick={state.reload}>Обновить спрос</button>
    {state.loading ? <Loading /> : state.error ? <ErrorState message={state.error} retry={state.reload} /> : !state.data?.length ? <Empty>Пока никто не отметил игры.</Empty> : <div className="stack">{state.data.map(item => <Card key={item.game.bggId}>
