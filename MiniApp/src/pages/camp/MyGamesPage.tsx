...
type RegistrationState = { campStatus: string; startDate?: string; endDate?: string; registration?: { registered: boolean; daysStaying: number; needsAccommodation: boolean; city?: string } };
...
function Registration({ community, state, done }: { community: Community; state: RegistrationState; done: () => void }) {
  ...
  const [days, setDays] = useState(state.registration?.daysStaying ?? duration); const [housing, setHousing] = useState(state.registration?.needsAccommodation ?? false); const [name, setName] = useState(""); const [city, setCity] = useState(state.registration?.city ?? ""); const [error, setError] = useState<string>(); const [busy, setBusy] = useState(false);
...
function ImportProgress({ community, id, close }: { community: Community; id: string; close: () => void }) {
  ...
  if (state.data.status === "Confirmed") { const overridable = state.data.draft?.items.filter(item => item.skipReason === "AlreadyInBaseCollection" && item.isOverridable) ?? []; return <Page title="Импорт завершён"><Notice kind="success">Выбранные игры сохранены.</Notice>{overridable.length > 0 && !state.data.overrideResolution && <BaseDuplicateResolution community={community} id={id} count={overridable.length} done={state.reload} />}{state.data.overrideResolution && <Notice>{state.data.overrideResolution === "AddPersonalCopies" ? "Ваши личные копии добавлены." : "Игры остаются только в базовой коллекции кэмпа."}</Notice>}<button onClick={close}>К моим играм</button></Page>; }
...
function ImportSelection({ community, id, items, done, cancel, cancellationError }: { community: Community; id: string; items: ImportDraftItem[]; done: () => void; cancel: () => void; cancellationError?: string }) {
  ...
  return <Page title="Выберите игры" subtitle={`Выбрано ${selected.size} из ${items.length}`} actions={<button className="danger ghost" onClick={cancel}>Отменить импорт</button>}><div className="row"><button onClick={() => setSelected(new Set(items.filter(x => !x.skipReason).map(x => `${x.itemType}-${x.bggId}`)))}>Выбрать все доступные</button><button onClick={() => setSelected(new Set())}>Очистить</button></div><Field label="Поиск"><input type="search" value={query} onChange={e => setQuery(e.target.value)} placeholder="Игра или дополнение" /></Field>{!hasMatches ? <Empty>Ничего не найдено.</Empty> : <div className="import-groups">{bases.map(base => { const children = expansions.filter(x => x.parentBggId === base.bggId && (!normalizedQuery || base.snapshot.name.toLowerCase().includes(normalizedQuery) || x.snapshot.name.toLowerCase().includes(normalizedQuery))); return <details open={normalizedQuery ? true : undefined} key={base.bggId}><summary><label className="check"><input type="checkbox" disabled={Boolean(base.skipReason)} checked={selected.has(`${base.itemType}-${base.bggId}`)} onChange={() => toggle(base)} /><ImportItemSummary item={base} /></label></summary>{children.map(exp => <label className={`check nested ${selected.has(`${exp.itemType}-${exp.bggId}`) && !hasBase(exp) ? "missing-base" : ""}`} key={exp.bggId}><input type="checkbox" disabled={Boolean(exp.skipReason)} checked={selected.has(`${exp.itemType}-${exp.bggId}`)} onChange={() => toggle(exp)} /><ImportItemSummary item={exp} />{selected.has(`${exp.itemType}-${exp.bggId}`) && !hasBase(exp) && <span className="missing-base-note">Нет базовой игры</span>}</label>)}</details>; })}{orphans.length > 0 && <section className="notice warning"><strong>Дополнения без базовой игры</strong><p className="muted">Это допустимо: их можно привезти отдельно.</p>{orphans.map(exp => <label className={`check nested ${selected.has(`${exp.itemType}-${exp.bggId}`) ? "missing-base" : ""}`} key={exp.bggId}><input type="checkbox" disabled={Boolean(exp.skipReason)} checked={selected.has(`${exp.itemType}-${exp.bggId}`)} onChange={() => toggle(exp)} /><ImportItemSummary item={exp} />{selected.has(`${exp.itemType}-${exp.bggId}`) && <span className="missing-base-note">Базовой игры нет в вашей коллекции</span>}</label>)}</section>}</div>}{(error || cancellationError) && <Notice kind="danger">{error ?? cancellationError}</Notice>}<button className="primary sticky-action" disabled={busy} onClick={confirm}>{busy ? "Сохраняем…" : `Сохранить (${selected.size})`}</button></Page>;
}
