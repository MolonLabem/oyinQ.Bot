...
export function ProfileCollectionPage({ community, bggAvailable }: { community?: Community; bggAvailable: boolean }) {
  ...
  const renderItem = (item: PersonalCollectionItem) => {
    const currentCommitment = campState.data?.find(x => x.bggId === item.bggId && x.itemType === item.itemType)?.commitment ?? "";
    return <div className="row contribution-row"><div className="media"><Cover src={item.snapshot.thumbnailImageUrl} name={item.snapshot.name} /><div><h2>{item.snapshot.name}</h2>{item.itemType === "Expansion" && <span className="muted"> Дополнение</span>}<GameMeta game={{ bggId: item.bggId, ...item.snapshot, expansions: [] }} /></div></div><div className="row">{community?.mode === "Camp" && <label>Для «{community?.name}»<select className={currentCommitment === "Bringing" ? "commitment-select success" : "commitment-select"} disabled={campState.loading || Boolean(campState.error)} aria-label={`Доступность ${item.snapshot.name}`} value={currentCommitment} onChange={e => void commitment(item, (e.target.value || null) as "Available" | "Bringing" | null)}><option value="">Не предлагаю для кэмпа</option><option value="Available">Могу привезти</option><option value="Bringing">Точно привезу</option></select></label>}<button className="danger ghost" onClick={() => remove(item)}>Убрать</button></div></div>;
  };
  ...
  return <Page as="section" title="Моя коллекция" subtitle="Ваши игры во всех сообществах. Доступность для кэмпа отмечается отдельно.">
    ...
    <section className="page-section"><h2>Добавить игры</h2><section className="content-section collection-tool"><h3>Импортировать коллекцию BGG</h3><p className="muted">Загрузите коллекцию из BGG и выберите игры, которыми владеете.</p><BggImportButton available={bggAvailable} start={startImport} /></section><div className="section-divider"><span>или</span></div>
    <section className="content-section collection-tool"><h3>Добавить игру или дополнение</h3><GamePicker bggAvailable={bggAvailable} selectionMode="item" selected={chosen} label="Найдите игру или дополнение по названию или BGG-ссылке"
      onSelect={game => { setChosen(game); setSelectedExpansions([]); }}
      onClear={() => { setChosen(undefined); setSelectedExpansions([]); }} />
      {chosen && <div className="selected-game"><div className="media"><Cover src={chosen.thumbnailImageUrl} name={chosen.name} /><div><h3>{chosen.name}</h3>{chosen.itemType === "Expansion" && <span className="muted">Дополнение</span>}<GameMeta game={chosen} /></div></div><ExpansionPicker expansions={chosen.expansions} selected={selectedExpansions} onChange={setSelectedExpansions} label="Дополнения в вашей коллекции" /><button className="primary" disabled={adding} onClick={addManual}>{adding ? "Добавляем…" : "Добавить в мои игры"}</button></div>}
    </section></section>
...
function ImportProgress({ base, community, id, close }: { base: string; community?: Community; id: string; close: (preserve?: boolean) => void }) {
  ...
  if (state.data.status === "Failed") return <Page as="section" title="Импорт BGG"><Badge tone={importStatusTone(state.data.status, state.data.stage)}>Ошибка</Badge><Notice kind="danger">Не удалось импортировать коллекцию BGG. Сохранённые игры не изменены; попробуйте ещё раз позже.</Notice>{actionError && <Notice kind="danger">{actionError}</Notice>}<button className="primary" disabled={actionBusy} onClick={retry}>{actionBusy ? "Запускаем повтор…" : "Повторить"}</button><button disabled={actionBusy} onClick={() => close()}>К моим играм</button></Page>;
  if (state.data.status === "Cancelled") return <Page as="section" title="Импорт отменён"><Badge tone={importStatusTone(state.data.status, state.data.stage)}>Отменён</Badge><Notice>Черновик не был применён.</Notice><button onClick={() => close()}>К моим играм</button></Page>;
  if (state.data.status !== "Completed" && state.data.status !== "Confirmed") return <Page as="section" title="Импорт BGG" subtitle={bggImportProgressText(state.data)}><Badge tone={importStatusTone(state.data.status, state.data.stage)}>{state.data.status === "Queued" ? "В очереди" : "Выполняется"}</Badge><Loading label="Импорт выполняется в фоне. Эту страницу можно закрыть." />{actionError && <Notice kind="danger">{actionError}</Notice>}<div className="row"><button disabled={actionBusy} onClick={() => close(true)}>Продолжить позже</button><button disabled={actionBusy} className="danger ghost" onClick={cancel}>{actionBusy ? "Отменяем…" : "Отменить импорт"}</button></div></Page>;
  if (state.data.status === "Confirmed") { const overridable = state.data.draft?.items.filter(item => item.skipReason === "AlreadyInBaseCollection" && item.isOverridable) ?? []; return <Page as="section" title="Импорт завершён"><Badge tone={importStatusTone(state.data.status, state.data.stage)}>Подтверждён</Badge><Notice kind="success">Выбранные игры сохранены.</Notice>{state.data.hasSelectedOverridableItems && !state.data.overrideResolution && <BaseDuplicateResolution community={community} id={id} count={overridable.length} done={state.reload} />}{state.data.overrideResolution && <Notice>{state.data.overrideResolution === "AddPersonalCopies" ? "Ваши личные копии добавлены." : "Игры остаются только в общей коллекции кэмпа."}</Notice>}<button onClick={() => close()}>К моим играм</button></Page>; }
