...
import { formatDate, formatInstant } from "../../app/format";
...
function BggImportButton({ available, start }: { available: boolean; start: (input: string) => void }) {
  ...
  return open ? <><Field label="Профиль BoardGameGeek" hint="Имя пользователя или ссылка на профиль"><input value={input} onChange={e => setInput(e.target.value)} placeholder="Например, John90" /></Field><button className="primary" disabled={!available || !input.trim()} onClick={() => start(input)}>Импортировать коллекцию</button></> : <button className="primary" disabled={!available} onClick={() => setOpen(true)}>Импортировать коллекцию</button>;
}
...
function ImportProgress({ base, community, id, close }: { base: string; community?: Community; id: string; close: (preserve?: boolean) => void }) {
  ...
  async function cancel() { if (!await telegram.confirm("Отменить импорт? Выбранные игры не будут сохранены.")) return; setActionError(undefined); try { await api(`${base}/imports/${id}/cancel`, json("POST", { communityKey: community?.key })); telegram.success("Импорт отменён"); close(); } catch (e) { setActionError(e instanceof Error ? e.message : String(e)); } }
  ...
  if (state.data.status === "Failed") return <Page as="section" title="Импорт BGG"><Notice kind="danger">Не удалось импортировать коллекцию BGG. Сохранённые игры не изменены; попробуйте ещё раз позже.</Notice><button onClick={() => api(`${base}/imports/${id}/retry`, json("POST", { communityKey: community?.key })).then(state.reload)}>Повторить</button><button onClick={() => close()}>К моим играм</button></Page>;
  ...
  if (state.data.status !== "Completed" && state.data.status !== "Confirmed") return <Page as="section" title="Импорт BGG" subtitle={state.data.progressTotal ? `Этап ${state.data.progressCurrent} из ${state.data.progressTotal}` : "В очереди"}><Loading label="Импорт выполняется в фоне. Эту страницу можно закрыть." />{actionError && <Notice kind="danger">{actionError}</Notice>}<div className="row"><button onClick={() => close(true)}>Продолжить позже</button><button className="danger ghost" onClick={cancel}>Отменить импорт</button></div></Page>;
  ...
  return <ImportSelection base={base} community={community} id={id} items={state.data.draft?.items ?? []} done={() => close()} cancel={cancel} cancellationError={actionError} />;
}

function ImportSelection({ base, community, id, items, done, cancel, cancellationError }: { base: string; community?: Community; id: string; items: ImportDraftItem[]; done: () => void; cancel: () => void; cancellationError?: string }) {
