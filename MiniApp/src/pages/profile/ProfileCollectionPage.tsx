...
export function ProfileCollectionPage({ community, bggAvailable, editRegistration, initialGameId }: { initialGameId?: number; community?: Community; bggAvailable: boolean; editRegistration?: () => void }) {
  ...
  return <>
    ...
    <section className="profile-collection stack" hidden={Boolean(importId && importOpen) || Boolean(detail)}>
      ...
      {community?.mode === "Camp" && <>
        <p className="muted">По умолчанию игры будут с вами во все дни участия{profile?.data?.myDates.length ? `: ${attendanceLabel(profile.data.myDates)}` : ""}. Для отдельных игр можно выбрать другие дни. <button className="person-link" onClick={editRegistration}>Изменить регистрацию</button></p>
