...
async function mount() { await act(async () => root.render(<CampProfileProvider key={camp.key} community={camp}><CampPrivacy community={camp} /><ProfileCollectionPage community={camp} bggAvailable /></CampProfileProvider>)); }
beforeEach(() => {
  ...
  contributions = []; settings = { canAct: true, shareCollection: false, shareWishes: false, myDates: ["2026-09-26", "2026-09-27"], declinedGameIds: [], suggestedGameIds: [42] };
...
it("keeps visibility above the collection and addition closed without losing the selected game", async () => {
  await mount();
  expect(host.querySelector("#collection-add")?.hasAttribute("hidden")).toBe(true);
  expect(visible(".collection-item")).toHaveLength(1);
  expect(host.textContent!.indexOf("Видимость для участников")).toBeLessThan(host.textContent!.indexOf("Игры ·"));
  expect(host.querySelectorAll('.camp-privacy input[type="checkbox"]')).toHaveLength(2);
...
it("does not show privacy as saved when the server rejects it", async () => {
  ...
  await mount(); const checkbox = host.querySelector<HTMLInputElement>('.camp-privacy input')!;
