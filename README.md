...
- BGG API token for BoardGameGeek XML API access
...
| `BGG_API_TOKEN` | BGG features | Bearer token for BoardGameGeek API access. |
...
2. Configure all required environment variables from the table above.
...
- BGG imports are stepped in chunks so a restart can resume from saved progress.
...
`GET /health` returns HTTP 200 when the ASP.NET Core process is running. It is a process-level health endpoint; it does not currently perform a PostgreSQL, Telegram, BGG, or Tesera dependency probe.
